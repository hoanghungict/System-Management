...
<head>
    ...
    <style>
        ...
        .task-info { background: white; padding: 15px; margin: 15px 0; border-left: 4px solid #9C27B0; }
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="task-info priority-{{ strtolower($priority) }} {{ $due_in_days <= 1 ? 'urgent' : '' }}">
                <h3>{{ $task_title }}</h3>
                ...
                @if($due_in_days !== null)
                    <p><strong>Due in:</strong> {{ $due_in_days }} days</p>
                @endif
            </div>
            ...
            @if($due_in_days <= 1)
                <div style="background: #ffebee; padding: 15px; margin: 15px 0; border-left: 4px solid #f44336;">
                    <h4>⚠️ Urgent Task</h4>
                    <p>This task is due very soon. Please prioritize it accordingly.</p>
                </div>
            @endif
