...
class ImportDataExcelController extends Controller
{
    ...
    public function ImportStudent(ImportStudentRequest $request): JsonResponse
    {
        try {
            ...
            Log::channel('daily')->info('Import job created', [
            ...
            Log::channel('daily')->error('Error creating import job', [
                'error' => $e->getMessage()
            ]);
    ...
    {
        try {
            $importJob = ImportJob::with(['failures', 'students' => function($query) {
                $query->limit(10); // Chỉ lấy 10 students đầu tiên
            }])->findOrFail($id);
